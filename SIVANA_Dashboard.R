...
server <- function(input, output, session) {
  ...
  output$categorization_plot <- renderPlot({
    ...
    if (!is.null(values$transformed_data)) {
      new_var_name <- paste0(input$var_to_categorize, "_cat")
      if (new_var_name %in% names(values$transformed_data)) {
        original_var <- values$transformed_data[[input$var_to_categorize]]
        cat_var <- values$transformed_data[[new_var_name]]
        ...
        ggplot(df_plot, aes(x = category, y = original, fill = category)) +
          geom_boxplot() +
          labs(title = paste("Kategorisasi", input$var_to_categorize),
  ...
  output$categorized_preview <- DT::renderDataTable({
    ...
    if (!is.null(values$transformed_data)) {
      new_var_name <- paste0(input$var_to_categorize, "_cat")
      if (new_var_name %in% names(values$transformed_data)) {
        preview_data <- values$transformed_data[, c(input$var_to_categorize, new_var_name)]
